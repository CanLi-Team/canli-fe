...
class practiceTestUpdated extends StatefulWidget {
  const practiceTestUpdated({Key? key}) : super(key: key);

  @override
  _practiceTestUpdated createState() => _practiceTestUpdated();
}

class _practiceTestUpdated extends State<practiceTestUpdated> {
  String title = "";
  ...
  List generateNoOfOptions(String op) {
    ...
    for (int i = 0; i < opt.length; i++) {
      Padding wid = generateOption(opt[i].toString().trim());
      pd.add(wid);
    }
    return pd;
  }
  ...
  Padding generateOption(String option) {
    return Padding(
      ...
      child: Card(
        ...
        shape: ((selected == true) && (option.trim() == (answers[index])))
            ? RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(12),
                side: BorderSide(color: Colors.green, width: 6))
            : ((selected == true) && option.trim() != answers[index])
                ? RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(12),
